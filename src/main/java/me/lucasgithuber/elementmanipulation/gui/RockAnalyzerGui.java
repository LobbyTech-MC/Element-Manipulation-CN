...
public abstract class RockAnalyzerGui extends SlimefunItem implements InventoryBlock, EnergyNetComponent, MachineProcessHolder<CraftingOperation> {
    ...
    protected void constructMenu(BlockMenuPreset preset) {
        ...
        for (int i : getInputBorder()) {
            preset.addItem(i, new CustomItemStack(Material.LIGHT_BLUE_STAINED_GLASS_PANE, "&b输入"), ChestMenuUtils.getEmptyClickHandler());
        }

        for (int i : getOutputBorder()) {
            preset.addItem(i, new CustomItemStack(Material.GREEN_STAINED_GLASS_PANE, "&a输出"), ChestMenuUtils.getEmptyClickHandler());
        }
